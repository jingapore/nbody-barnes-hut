...
constexpr int num_sub_cells = 1 << DIM_SIZE;
...
struct Cell {
    ...
    Cell * subcells[num_sub_cells]; // eight subcells maximum
    bool inserted = false;
    const Body * b = nullptr;
    bool send_as_leaf = false;
    int list_idx = -1; // computed during `cells_to_send`
    int parent_idx = -1; // computed during `cells_to_send`, it's an indicated whehter a cell is root, which is useful for partner that receives a root, where the partner needs to then make more changes to its local tree
};
...
public:
    ...
    */
    Tree(const double * min_bounds, const double * max_bounds, double theta, double g, double r);
    ...
    */
    array<double, DIM_SIZE> compute_force(const Body * b);
...
private:
    ...
    */
    ...
    @param g: gravitational constant
    */
    array<double, DIM_SIZE> compute_force(const Cell * cell, const Body * b);
    ...
    */
    ...
    /*
    minimum value of distance between bodies so as to ensure that distance doesn't get rounded down to zero
    this is used in eval_force
    */
    double rlimit;
