...
bool is_above_split(int rank, int n_processes){
    // let's work out what happens when n_processes = 8
    // rel_bit = 3, and we will shift each rank by 2 bits, essentially taking the 3rd bit (i.e. 100=4)
    // rank 0 -> not_above, rank 1 -> not_above... rank 4 -> above .. rank 7 --> above
    // and then for the next split, we shift more bits and end up discriminating between processors on lower bits, where above/below is based 
    // on whether a process has a rank that is on the "upper half" or "lower half"
    // TODO: figure out whether to take floor or ceil when n_processes not equal to pow 2
...
int get_partner(int rank, int n_processes){
    // note: if we use gray code, we may need to change this
    // let's figure out how partners are assigned
    // n_processes = n_proc_left = n_processors / pow(2,i)
    // let's look at the 2nd last split for 8 processors where ranks 0/1/2/3 are in the same split, 4/5/6/7 are in the same split
    // relbit will be 2, since n_processes = 1, and we take rank XOR 100
    // so 000 will be paired with 100 (or 4), 001 will be paired with 101 (or 5)
    int rel_bit = (int) log2(n_processes);
    int partner = rank ^ (1 << (rel_bit - 1));
    return partner; 
}

int split_coord(const array<double, DIM_SIZE> & min, const array<double, DIM_SIZE> & max){
    double max_diff = -1;
    int coord;
...
void orb(vector<Body> & bodies, bound_vec & bounds,
    ...
    array<double, DIM_SIZE> min_bounds, max_bounds, min, max, other_min, other_max;

    /* Copy over global min and max */
    std::copy(global_min, &global_min[DIM_SIZE], std::begin(min));  
    std::copy(global_max, &global_max[DIM_SIZE], std::begin(max));  
    ...
    for(int i = 0; i < n_splits; i++){
        ...
        f = [&](double split){return weight_frac(bodies, split, coord, subset_comm) - 0.5;}; // checking if `tot_work_below / (tot_work_above + tot_work_below) = 0.5`
        split = bisection(min[coord], max[coord], f); // max_iter set to 200 by default in `util.h`
...
void global_minmax(const vector<Body> bodies, double * min, double * max){
    ...
    for (int c=0; c<DIM_SIZE; c++) {
        min[c] = std::numeric_limits<double>::infinity();
        max[c] = -min[c];
    }
