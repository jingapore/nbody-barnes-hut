...
Tree::Tree(const double * min_bounds, const double * max_bounds, double theta, double g, double r){
    ...
    /* create root cell */
    root = new Cell();
    copy(min_bounds, &min_bounds[DIM_SIZE], root->min_bounds);
    copy(max_bounds, &max_bounds[DIM_SIZE], root->max_bounds);
    root->m = 0;
    fill(root->subcells, &root->subcells[num_sub_cells], nullptr);
}
...
void Tree::insert_emptycell(Cell * cell, const double * min_bounds, const double * max_bounds){
    ...
    for(int i = 0; i < num_sub_cells; i++){
        ...
        /* if bounds does not fit in subcell, create a new one */
            ...
            Cell * subcell = new Cell();
            copy(min_bounds, &min_bounds[DIM_SIZE], subcell->min_bounds);
            copy(max_bounds, &max_bounds[DIM_SIZE], subcell->max_bounds);
            subcell->m = 0;
            fill(subcell->subcells, &subcell->subcells[num_sub_cells], nullptr);
            cell->subcells[i] = subcell;
            return;
        }
    }
...
void Tree::insert_cell(Cell * cell,
    ...
    for(int i = 0; i < num_sub_cells; i++){
        ...
        /* if bounds does not fit in subcell, create a new one */
            ...
            copy(min_bounds, &min_bounds[DIM_SIZE], subcell->min_bounds);
            copy(max_bounds, &max_bounds[DIM_SIZE], subcell->max_bounds);
            copy(rm, &rm[DIM_SIZE], subcell->rm);
            subcell->m = m;
...
string Tree::subtree_str(Cell * cell, bool fulltree) const{
    ...
    int count = 0;
    if(!fulltree){
        for(count = 0; count < num_sub_cells; count++){
...
bool Tree::opening_criterion(const Cell * cell, const double * min_bounds, const double * max_bounds){
    ...
    double v2 = pow(cell_volume(cell),2/(double)DIM_SIZE); //TODO: figure out what is this.

    /* opening criterion rule */
    if(m_theta * m_theta * d2 < v2){ // default for m_theta is 1
        return true;
    }
    else{
        return false;
...
array<double, DIM_SIZE> Tree::compute_force(const Body * b){
    return compute_force(root, b);
}

array<double, DIM_SIZE> Tree::compute_force(const Cell * cell, const Body * b){
    ...
    if(cell->m != 0 and (!opening_criterion(cell, b->pos, b->pos) or cell->subcells[0] == nullptr)){
        if(cell->b != b){
            /* evaluate force */
            return eval_force(cell->rm, cell->m, b->pos, b->m, grav, rlimit);
        }
    ...
    array<double, DIM_SIZE> ftot = {};
    for(int i = 0; i < num_sub_cells; i++){
        if(cell->subcells[i]!=nullptr){
            /* call force computation recursively */
            array<double, DIM_SIZE> f = compute_force(cell->subcells[i], b);
