...
std::pair<std::vector<Body>, int> read_bodies(const char * filename, MPI_Comm comm, int& nbodies, const int& ndims){
    int rank, size;
    ...
    while (std::getline(infile, line)){
        ...
        if (!(iss >> idx >> x >> y >> z >> m >> vx >> vy >> vz)) { break; } // error
        if((idx % size) == rank){
            bodies.push_back(Body{idx, {x, y, z}, {vx, vy, vz}, m, 1});
        #elif DIM_SIZE == 2
        double x, y, m, vx, vy;
        if (!(iss >> idx >> x >> y >> m >> vx >> vy)) { break; } // error
        if((idx % size) == rank){
            bodies.push_back(Body{idx, {x, y}, {vx, vy}, m, 1});
        }
        #else
        #error "Unsupported DIM_SIZE"
        #endif
    }
    infile.close();
    ...
    return std::make_pair(bodies, idx);
}
...
void write_bodies(const char * filename, const std::vector<Body> & bodies, MPI_Comm comm, bool overwrite){
    ...
    if(rank != 0){
        MPI_Status status;
        int x;
        // this blocks so that there's only 1 proc reading the file at any time
        MPI_Recv(&x, 1, MPI_INT, rank - 1, 0,
                MPI_COMM_WORLD, &status);
...
void write_summary(InputParser ip, int N, int P){
    ...
    file << "barnes hut approximation constant (theta): " << ip.bh_approx_constant() << std::endl;
